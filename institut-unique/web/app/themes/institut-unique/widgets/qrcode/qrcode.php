 ...
 */
add_action( 'widgets_init', 'jacqueline_widget_qrcode_load' );

/**
 * Register our widget.
 */
function jacqueline_widget_qrcode_load() {
	register_widget('jacqueline_widget_qrcode');
}

/**
 * QRCode Widget class.
 */
class jacqueline_widget_qrcode extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array('classname' => 'widget_qrcode', 'description' => esc_html__('Generate QRCode with your personal data or with any text (links)', 'jacqueline'));

		/* Widget control settings. */
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'jacqueline_widget_qrcode');

		/* Create the widget. */
		parent::__construct( 'jacqueline_widget_qrcode', esc_html__('Jacqueline - QRCode generator', 'jacqueline'), $widget_ops, $control_ops );
	...
	function widget($args, $instance) {
		...
		if ($show_personal) 
			$output .= '
					...
					' . ($show_what==1 
						? '<p class="user_name odd first"><span class="theme_text">' . esc_html__('Name:', 'jacqueline') . '</span> <span class="theme_info">' . ($ufname) . ' ' . ($ulname) . '<span></p>'
							. ($ucompany ? '<p class="user_company even"><span class="theme_text">' . esc_html__('Company:', 'jacqueline') . '</span> <span class="theme_info">' . ($ucompany) . '<span></p>' : '')
							. ($uphone ? '<p class="user_phone odd"><span class="theme_text">' . esc_html__('Phone:', 'jacqueline') . '</span> <span class="theme_info">' . ($uphone) . '<span></p>' : '')
							. ($uemail ? '<p class="user_email even"><span class="theme_text">' . esc_html__('E-mail:', 'jacqueline') . '</span> <a href="' . esc_url('mailto:'.($uemail)) . '">' . ($uemail) . '</a></p>' : '')
							. ($usite ? '<p class="user_site odd"><span class="theme_text">' . esc_html__('Site:', 'jacqueline') . '</span> <a href="' . esc_url($usite) . '" target="_blank">' . ($usite) . '</a></p>' : '')
						: $text)
						. '
					</div>
					';
	...
	function form($instance) {
		...
		jacqueline_enqueue_style( 'widget-qrcode-style',  jacqueline_get_file_url('widgets/qrcode/qrcode-admin.css'), array(), null );
		jacqueline_enqueue_script('jquery-ui-tabs', false, array('jquery','jquery-ui-core'), null, true);
		jacqueline_enqueue_script('widget-qrcode-script', jacqueline_get_file_url('widgets/qrcode/jquery.qrcode-0.6.0.min.js'), array(), null, false);
		//jacqueline_enqueue_style('color-picker',        jacqueline_get_file_url('js/colorpicker/colorpicker.css'), array(), null);
		//jacqueline_enqueue_script('color-picker',       jacqueline_get_file_url('js/colorpicker/colorpicker.js'), array('jquery'),null,true);

		/* Set up some default widget settings. */
		$address = explode(',', jacqueline_get_theme_option('user_address'));
		$defaults = array(
			'title' => '', 
			'description' => esc_html__('QR Code Generator (for your vcard)', 'jacqueline'),
			'ulname' => '', 
			'ufname' => '', 
			'ucompany' => '', 
			'uaddr' => '', 
		...
		<div class="widget_qrcode">
        	<div class="qrcode_tabs">
                <ul class="tabs">
                    <li class="first"><a href="#tab_settings"><?php esc_html_e('Settings', 'jacqueline'); ?></a></li>
                    <li><a href="#tab_fields" onmousedown="jacqueline_qrcode_init()"><?php esc_html_e('Personal Data', 'jacqueline'); ?></a></li>
                    <li><a href="#tab_text" onmousedown="jacqueline_qrcode_init()"><?php esc_html_e('Any Text', 'jacqueline'); ?></a></li>
                </ul>
                <div id="tab_settings" class="tab_content tab_settings">
                    <p>
                        ...
                        <input class="fld_title" onfocus="jacqueline_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($instance['title']); ?>" class="widgets_param_fullwidth" />
                    </p>
                    <p>
                        <label><?php esc_html_e('Show as QR Code:', 'jacqueline'); ?></label><br />
                        <input class="fld_show_what" onfocus="jacqueline_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="1" type="radio" <?php echo (1==$instance['show_what'] ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1"> <?php esc_html_e('Personal VCard', 'jacqueline'); ?></label>
                        <input class="fld_show_what" onfocus="jacqueline_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="0" type="radio" <?php echo (0==$instance['show_what'] ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0"> <?php esc_html_e('Any text', 'jacqueline'); ?></label>
                    </p>
                    <p>
                        <input class="fld_show_personal" onfocus="jacqueline_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_personal')); ?>" name="<?php echo esc_attr($this->get_field_name('show_personal')); ?>" value="1" type="checkbox" <?php echo (1==$instance['show_personal'] ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_personal')); ?>"><?php esc_html_e('Show data under QR Code:', 'jacqueline'); ?></label>
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('width')); ?>"><?php esc_html_e('Width:', 'jacqueline'); ?></label>
                        <input onmousedown="jacqueline_qrcode_init()" onfocus="jacqueline_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('width')); ?>" name="<?php echo esc_attr($this->get_field_name('width')); ?>" value="<?php echo esc_attr($instance['width']); ?>" class="widgets_param_fullwidth fld_width" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('color')); ?>"><?php esc_html_e('Color:', 'jacqueline'); ?></label>
                        <input onmousedown="jacqueline_qrcode_init()" onfocus="jacqueline_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('color')); ?>" name="<?php echo esc_attr($this->get_field_name('color')); ?>" value="<?php echo esc_attr($instance['color']); ?>" class="widgets_param_fullwidth iColorPicker fld_color" style="background-color:<?php echo esc_attr($instance['color']); ?>" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('bg')); ?>"><?php esc_html_e('Bg color:', 'jacqueline'); ?></label>
                        <input onmousedown="jacqueline_qrcode_init()" onfocus="jacqueline_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('bg')); ?>" name="<?php echo esc_attr($this->get_field_name('bg')); ?>" value="<?php echo esc_attr($instance['bg']); ?>" style="background-color:<?php echo esc_attr($instance['bg']); ?>" class="widgets_param_fullwidth iColorPicker fld_bg" />
                    </p>
                </div>
                <div id="tab_fields" class="tab_content tab_personal">
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ulname')); ?>"><?php esc_html_e('Last name:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ufname')); ?>"><?php esc_html_e('First name:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ucompany')); ?>"><?php esc_html_e('Company:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('uphone')); ?>"><?php esc_html_e('Phone:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('uaddr')); ?>"><?php esc_html_e('Address:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ucity')); ?>"><?php esc_html_e('City:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('upostcode')); ?>"><?php esc_html_e('Post code:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ucountry')); ?>"><?php esc_html_e('Country:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('uemail')); ?>"><?php esc_html_e('E-mail:', 'jacqueline'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('usite')); ?>"><?php esc_html_e('Web Site URL:', 'jacqueline'); ?></label>
                ...
                <div id="tab_text" class="tab_content tab_text">
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('fld_text')); ?>"><?php esc_html_e('Text to show as QR Code:', 'jacqueline'); ?></label>
            ...
            <p>
                <input class="fld_button_draw" id="<?php echo esc_attr($this->get_field_id('button_draw')); ?>" name="<?php echo esc_attr($this->get_field_name('button_draw')); ?>" value="<?php esc_attr_e('Update', 'jacqueline'); ?>" type="button" />
                ...
                <label for="<?php echo esc_attr($this->get_field_id('auto_draw')); ?>"> <?php esc_html_e('Auto', 'jacqueline'); ?></label>
            </p>
...
<?php if ($instance['show_personal']==1) { ?>
				...
				<?php if ($instance['show_what']==1) { ?>
                    <li class="user_name odd first"><?php echo  esc_html__('Name:', 'jacqueline') . ' ' . ($instance['ufname']) . ' ' . ($instance['ulname']); ?></li>
                    <?php 
						echo  ($instance['ucompany'] ? '<li class="user_company even">' . esc_html__('Company:', 'jacqueline') . ' ' . ($instance['ucompany']) . '</li>' : '')
							. ($instance['uphone'] ? '<li class="user_phone odd">' . esc_html__('Phone:', 'jacqueline') . ' ' . ($instance['uphone']) . '</li>' : '')
							. ($instance['uemail'] ? '<li class="user_email even">' . esc_html__('E-mail:', 'jacqueline') . ' ' . '<a href="' . esc_url('mailto:'.($instance['uemail'])) . '">' . ($instance['uemail']) . '</a></li>' : '')
							. ($instance['usite'] ? '<li class="user_site odd">' . esc_html__('Site:', 'jacqueline') . ' ' . '<a href="' . esc_url($instance['usite']) . '" target="_blank">' . ($instance['usite']) . '</a></li>' : '');
					?>
				<?php } else { ?>
...
<?php } ?>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function(){
				jacqueline_qrcode_init();
				jacqueline_color_picker();
            });
			function jacqueline_qrcode_init() {
            ...
            function jacqueline_qrcode_update(widget) {
				jacqueline_qrcode_show(widget, {
			...
			function jacqueline_qrcode_show(widget, vc, opt) {
				if (opt.show_what==1) {
			...
			if (!window.jacqueline_set_color_picker) {
				function jacqueline_set_color_picker(id_picker) {
	...
	function load_scripts() {
		...
		jacqueline_enqueue_script('widget-qrcode-script', 		jacqueline_get_file_url('widgets/qrcode/jquery.qrcode-0.6.0.min.js'), array('jquery'), null, true);
	}
}
?>