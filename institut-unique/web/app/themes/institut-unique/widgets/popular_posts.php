...
// Theme init
if (!function_exists('jacqueline_widget_popular_posts_theme_setup')) {
	add_action( 'jacqueline_action_before_init_theme', 'jacqueline_widget_popular_posts_theme_setup', 1 );
	function jacqueline_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('jacqueline_action_shortcodes_list',	'jacqueline_widget_popular_posts_reg_shortcodes');
		if (function_exists('jacqueline_exists_visual_composer') && jacqueline_exists_visual_composer())
			add_action('jacqueline_action_shortcodes_list_vc','jacqueline_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('jacqueline_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'jacqueline_widget_popular_posts_load' );
	function jacqueline_widget_popular_posts_load() {
		register_widget('jacqueline_widget_popular_posts');
	}
}

// Widget Class
class jacqueline_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'jacqueline'));
		parent::__construct( 'jacqueline_widget_popular_posts', esc_html__('Jacqueline - Most Popular & Commented Posts', 'jacqueline'), $widget_ops );

		// Add thumb sizes into list
		jacqueline_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'title'=>esc_html__('Widgets', 'jacqueline') ) );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? jacqueline_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = 'jacqueline_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = 'jacqueline_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		$posts_types = jacqueline_get_list_posts_types(false);
		$categories = jacqueline_get_list_terms(false, jacqueline_get_taxonomy_categories_by_post_type($post_type));
		?>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Widget title:', 'jacqueline'); ?></label>
			<input id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($title); ?>" class="widgets_param_fullwidth" />
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'jacqueline'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'jacqueline'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'jacqueline'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php esc_html_e('Post type:', 'jacqueline'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('category')); ?>"><?php esc_html_e('Category:', 'jacqueline'); ?></label>
			<select id="<?php echo esc_attr($this->get_field_id('category')); ?>" name="<?php echo esc_attr($this->get_field_name('category')); ?>" class="widgets_param_fullwidth">
				<option value="0"><?php esc_html_e('-- Any category --', 'jacqueline'); ?></option> 
				<?php
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Number posts to show:', 'jacqueline'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_1"><?php esc_html_e('Show post image:', 'jacqueline'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_1"><?php esc_html_e('Show', 'jacqueline'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_0"><?php esc_html_e('Hide', 'jacqueline'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_1"><?php esc_html_e('Show post author:', 'jacqueline'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_1"><?php esc_html_e('Show', 'jacqueline'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_0"><?php esc_html_e('Hide', 'jacqueline'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_1"><?php esc_html_e('Show post date:', 'jacqueline'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_1"><?php esc_html_e('Show', 'jacqueline'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_0"><?php esc_html_e('Hide', 'jacqueline'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show post counters:', 'jacqueline'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show', 'jacqueline'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_0"><?php esc_html_e('Hide', 'jacqueline'); ?></label>
		</p>
...
*/
if ( !function_exists( 'jacqueline_sc_widget_popular_posts' ) ) {
	function jacqueline_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'jacqueline_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (jacqueline_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, jacqueline_prepare_widgets_args(jacqueline_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('jacqueline_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	jacqueline_require_shortcode("trx_widget_popular_posts", "jacqueline_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('jacqueline_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('jacqueline_action_shortcodes_list_vc','jacqueline_widget_popular_posts_reg_shortcodes_vc');
	function jacqueline_widget_popular_posts_reg_shortcodes_vc() {
		
		$posts_types = jacqueline_get_list_posts_types(false);
		$categories = jacqueline_get_list_terms(false, jacqueline_get_taxonomy_categories_by_post_type('post'));

		vc_map( array(
				"base" => "trx_widget_popular_posts",
				"name" => esc_html__("Widget Popular Posts", 'jacqueline'),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", 'jacqueline') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", 'jacqueline'),
						"description" => wp_kses_data( __("Most popular tab title", 'jacqueline') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", 'jacqueline'),
						"description" => wp_kses_data( __("Most commented tab title", 'jacqueline') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", 'jacqueline'),
						"description" => wp_kses_data( __("Most liked tab title", 'jacqueline') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "number",
						"heading" => esc_html__("Number posts to show", 'jacqueline'),
						"description" => wp_kses_data( __("How many posts display in widget?", 'jacqueline') ),
						"admin_label" => true,
						"class" => "",
						"value" => "4",
						"type" => "textfield"
					...
					array(
						"param_name" => "cat",
						"heading" => esc_html__("Parent category", 'jacqueline'),
						"description" => wp_kses_data( __("Select parent category. If empty - show posts from any category", 'jacqueline') ),
						"class" => "",
						"value" => array_flip(jacqueline_array_merge(array(0 => esc_html__('- Select category -', 'jacqueline')), $categories)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "show_image",
						"heading" => esc_html__("Show post's image", 'jacqueline'),
						"description" => wp_kses_data( __("Do you want display post's featured image?", 'jacqueline') ),
					...
					array(
						...
						"heading" => esc_html__("Show post's author", 'jacqueline'),
						"description" => wp_kses_data( __("Do you want display post's author?", 'jacqueline') ),
						"group" => esc_html__('Details', 'jacqueline'),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show post's date", 'jacqueline'),
						"description" => wp_kses_data( __("Do you want display post's publish date?", 'jacqueline') ),
						"group" => esc_html__('Details', 'jacqueline'),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show post's counters", 'jacqueline'),
						"description" => wp_kses_data( __("Do you want display post's counters?", 'jacqueline') ),
						"group" => esc_html__('Details', 'jacqueline'),
						"class" => "",
						"std" => 1,
