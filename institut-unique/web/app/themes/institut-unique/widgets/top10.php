...
// Theme init
if (!function_exists('jacqueline_widget_top10_theme_setup')) {
	add_action( 'jacqueline_action_before_init_theme', 'jacqueline_widget_top10_theme_setup', 1 );
	function jacqueline_widget_top10_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('jacqueline_action_shortcodes_list',	'jacqueline_widget_top10_reg_shortcodes');
		if (function_exists('jacqueline_exists_visual_composer') && jacqueline_exists_visual_composer())
			add_action('jacqueline_action_shortcodes_list_vc','jacqueline_widget_top10_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('jacqueline_widget_top10_load')) {
	add_action( 'widgets_init', 'jacqueline_widget_top10_load' );
	function jacqueline_widget_top10_load() {
		register_widget('jacqueline_widget_top10');
	}
}

// Widget Class
class jacqueline_widget_top10 extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_top10', 'description' => esc_html__('Top 10 posts by average reviews marks (by author and users)', 'jacqueline'));
		parent::__construct( 'jacqueline_widget_top10', esc_html__('Jacqueline - Top 10 Posts', 'jacqueline'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		for ($i=0; $i<2; $i++) {
			...
			$post_rating = 'jacqueline_reviews_avg'.($i==0 ? '' : '2');
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_author')); ?>"><?php esc_html_e('Author rating tab title:', 'jacqueline'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_users')); ?>"><?php esc_html_e('Users rating tab title:', 'jacqueline'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_2"><?php esc_html_e('As stars', 'jacqueline'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('As icon', 'jacqueline'); ?></label>
...
if ( !function_exists( 'jacqueline_sc_widget_top10' ) ) {
	function jacqueline_sc_widget_top10($atts, $content=null){	
		...
		extract($atts);
		$type = 'jacqueline_widget_top10';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_top10' 
								. (jacqueline_exists_visual_composer() ? ' vc_widget_top10 wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, jacqueline_prepare_widgets_args(jacqueline_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_top10', 'widget_top10') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('jacqueline_shortcode_output', $output, 'trx_widget_top10', $atts, $content);
	}
	jacqueline_require_shortcode("trx_widget_top10", "jacqueline_sc_widget_top10");
}


// Add [trx_widget_top10] in the VC shortcodes list
if (!function_exists('jacqueline_widget_top10_reg_shortcodes_vc')) {
	//add_action('jacqueline_action_shortcodes_list_vc','jacqueline_widget_top10_reg_shortcodes_vc');
	function jacqueline_widget_top10_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Top10 Reviews", 'jacqueline'),
				"description" => wp_kses_data( __("Insert Top10 reviews list with thumbs and post's meta", 'jacqueline') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Author's tab title", 'jacqueline'),
						"description" => wp_kses_data( __("Title for the tab with authors's reviews", 'jacqueline') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("User's tab title", 'jacqueline'),
						"description" => wp_kses_data( __("Title for the tab with authors's reviews", 'jacqueline') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"value" => array_flip(array(
							'2' => esc_html__('As stars', 'jacqueline'),
							'1' => esc_html__('As text', 'jacqueline'),
							'0' => esc_html__('Hide', 'jacqueline')
						)),
						"type" => "dropdown"
					),
