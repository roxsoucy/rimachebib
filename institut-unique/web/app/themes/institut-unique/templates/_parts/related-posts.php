...
if (jacqueline_get_custom_option("show_post_related") == 'yes') {

	if (empty($body_style) && !empty($post_options['body_style'])) $body_style = $post_options['body_style'];
	if (empty($body_style)) $body_style = jacqueline_get_custom_option('body_style');
	...
	if ($number > 0) {
		...
		$need_dummy = false;
		?>
		<section class="related_wrap<?php echo ((int) $columns > 1 ? '' : ' scroll_wrap') . esc_attr(jacqueline_get_template_property('related', 'container_classes')); ?>">

			<?php if ($need_wrap) jacqueline_open_wrapper('<div class="content_wrap">'); ?>
			
			<h2 class="section_title"><?php echo apply_filters('jacqueline_filter_related_posts_title', esc_html__('Related Posts', 'jacqueline'), $post_data['post_type']); ?></h2>
			...
			<?php } else if (jacqueline_get_template_property('related', 'need_columns')) { ?>
			...
			<?php if ($need_wrap) jacqueline_close_wrapper(); ?>

		</section>
		<?php
	}
	if ($body_style!='fullscreen' && !$sidebar_present) jacqueline_open_all_wrappers();
}
