...
if ( !function_exists( 'jacqueline_template_single_players_output' ) ) {
	function jacqueline_template_single_players_output($post_options, $post_data) {
		...
		jacqueline_open_wrapper('<section class="post_content'.(!$post_data['post_protected'] && $post_data['post_edit_enable'] ? ' '.esc_attr('post_content_editor_present') : '').'" itemprop="articleBody">');
		...
		if(!empty($post_meta['player_socials'])){ 
			...
			echo trim(jacqueline_do_shortcode('[trx_socials size="tiny" shape="round" socials="'.esc_attr($socials).'"][/trx_socials]'));
		}
		...
		jacqueline_close_wrapper();	// .post_item
