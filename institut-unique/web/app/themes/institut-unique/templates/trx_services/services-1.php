		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_services_item_content">
					...
					?>
						<div class="sc_services_item_category"><?php echo the_terms( $post_data['post_id'], 'services_group', '', ', ' ); ?></div>
					<?php
					}
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if ($post_data['post_excerpt'] && $post_options['tag_type']=='icons') {
							...
							if (!empty($post_data['post_link']) && !jacqueline_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore sc_button sc_button_style_filled sc_button_size_medium">
									<div><span class="first"><?php echo trim($post_options['readmore']); ?></span><span class="second"><?php echo trim($post_options['readmore']); ?></span></div>
								</a><?php
							}
						}
						?>
