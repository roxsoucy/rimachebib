...
if (!function_exists('jacqueline_testimonial_theme_setup')) {
	...
	function jacqueline_testimonial_theme_setup() {
		...
		add_action('add_meta_boxes',		'jacqueline_testimonial_add_meta_box');
		...
		// Meta box fields
		jacqueline_storage_set('testimonial_meta_box', array(
...
// Add meta box
if (!function_exists('jacqueline_testimonial_add_meta_box')) {
	//add_action('add_meta_boxes', 'jacqueline_testimonial_add_meta_box');
	function jacqueline_testimonial_add_meta_box() {
		$mb = jacqueline_storage_get('testimonial_meta_box');
		add_meta_box($mb['id'], $mb['title'], 'jacqueline_testimonial_show_meta_box', $mb['page'], $mb['context'], $mb['priority']);
	}
}

// Callback function to show fields in meta box
if (!function_exists('jacqueline_testimonial_show_meta_box')) {
	function jacqueline_testimonial_show_meta_box() {
		global $post;

		// Use nonce for verification
		echo '<input type="hidden" name="meta_box_testimonial_nonce" value="'.esc_attr(wp_create_nonce(admin_url())).'" />';
		
		$data = get_post_meta($post->ID, 'jacqueline_testimonial_data', true);
	
		$fields = jacqueline_storage_get_array('testimonial_meta_box', 'fields');
		?>
...
if (!function_exists('jacqueline_testimonial_save_data')) {
	...
	function jacqueline_testimonial_save_data($post_id) {
		// verify nonce
		if ( !wp_verify_nonce( jacqueline_get_value_gp('meta_box_testimonial_nonce'), admin_url() ) )
			return $post_id;
		...
		update_post_meta($post_id, 'jacqueline_testimonial_data', $data);
	}
}
	...
	function jacqueline_sc_testimonials($atts, $content=null){	
		...
		if (jacqueline_param_is_on($custom) && $content) {
			...
			while ( $query->have_posts() ) { 
				...
				$post_meta = get_post_meta($post_data['post_id'], 'jacqueline_testimonial_data', true);
...
if (!function_exists('jacqueline_testimonials_reg_shortcodes')) {
	//add_filter('jacqueline_action_shortcodes_list',	'jacqueline_testimonials_reg_shortcodes');
...
if (!function_exists('jacqueline_testimonials_reg_shortcodes_vc')) {
	//add_filter('jacqueline_action_shortcodes_list_vc',	'jacqueline_testimonials_reg_shortcodes_vc');
