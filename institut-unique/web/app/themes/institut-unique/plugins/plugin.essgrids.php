...
if (!function_exists('jacqueline_essgrids_theme_setup')) {
	...
	function jacqueline_essgrids_theme_setup() {
		...
		if (jacqueline_exists_essgrids()) {
			if (is_admin()) {
				add_filter( 'jacqueline_filter_importer_options',			'jacqueline_essgrids_importer_set_options', 10, 1 );
				add_action( 'jacqueline_action_importer_params',			'jacqueline_essgrids_importer_show_params', 10, 1 );
				add_action( 'jacqueline_action_importer_import',			'jacqueline_essgrids_importer_import', 10, 2 );
				add_action( 'jacqueline_action_importer_import_fields',	'jacqueline_essgrids_importer_import_fields', 10, 1 );
			}
		}
		if (is_admin()) {
			add_filter( 'jacqueline_filter_importer_required_plugins',	'jacqueline_essgrids_importer_required_plugins', 10, 2 );
...
if ( !function_exists( 'jacqueline_essgrids_required_plugins' ) ) {
	...
	function jacqueline_essgrids_required_plugins($list=array()) {
		if (in_array('essgrids', jacqueline_storage_get('required_plugins'))) {
...
if ( !function_exists( 'jacqueline_essgrids_importer_required_plugins' ) ) {
	//add_filter( 'jacqueline_filter_importer_required_plugins',	'jacqueline_essgrids_importer_required_plugins', 10, 2 );
	function jacqueline_essgrids_importer_required_plugins($not_installed='', $list='') {
		//if (in_array('essgrids', jacqueline_storage_get('required_plugins')) && !jacqueline_exists_essgrids() )
		if (jacqueline_strpos($list, 'essgrids')!==false && !jacqueline_exists_essgrids() )
...
if ( !function_exists( 'jacqueline_essgrids_importer_set_options' ) ) {
	//add_filter( 'jacqueline_filter_importer_options',	'jacqueline_essgrids_importer_set_options', 10, 1 );
	function jacqueline_essgrids_importer_set_options($options=array()) {
		if ( in_array('essgrids', jacqueline_storage_get('required_plugins')) && jacqueline_exists_essgrids() ) {
...
if ( !function_exists( 'jacqueline_essgrids_importer_show_params' ) ) {
	//add_action( 'jacqueline_action_importer_params',	'jacqueline_essgrids_importer_show_params', 10, 1 );
	function jacqueline_essgrids_importer_show_params($importer) {
		?>
		<input type="checkbox" <?php echo in_array('essgrids', jacqueline_storage_get('required_plugins')) && $importer->options['plugins_initial_state'] 
											? 'checked="checked"' 
											: ''; ?> value="1" name="import_essgrids" id="import_essgrids" /> <label for="import_essgrids"><?php esc_html_e('Import Ess.Grids', 'jacqueline'); ?></label><br>
		<?php
	}
}

// Import posts
if ( !function_exists( 'jacqueline_essgrids_importer_import' ) ) {
	//add_action( 'jacqueline_action_importer_import',	'jacqueline_essgrids_importer_import', 10, 2 );
	function jacqueline_essgrids_importer_import($importer, $action) {
		if ( $action == 'import_essgrids' ) {
			$attempt = !empty($_POST['attempt']) ? (int) $_POST['attempt']+1 : 1;
			if ($importer->options['debug']) 
				dfl( sprintf(esc_html__('Import Essential Grid "%s". Attempt %d.', 'jacqueline'), $file, $attempt) );
			$data = jacqueline_get_local_or_remote_file($importer->options['files'][$importer->options['demo_type']]['file_with_essgrid']);
			if (empty($data)) {
				$msg = sprintf(esc_html__('Essential Grid "%s" import error. Attempt %d.', 'jacqueline'), $importer->options['files'][$importer->options['demo_type']]['file_with_essgrid'], $attempt);
				if ($attempt < 3) {
				...
				try {
					...
					if ($importer->options['debug']) 
						dfl( esc_html__('Essential Grid import complete', 'jacqueline') );
							
				} catch (Exception $d) {
					$msg = sprintf(esc_html__('Essential Grid import error: %s', 'jacqueline'), $d->getMessage());
					$importer->response['error'] = $msg;
					if ($importer->options['debug']) 
						dfl( $msg );
...
if ( !function_exists( 'jacqueline_essgrids_importer_import_fields' ) ) {
	//add_action( 'jacqueline_action_importer_import_fields',	'jacqueline_essgrids_importer_import_fields', 10, 1 );
	function jacqueline_essgrids_importer_import_fields($importer) {
		?>
		<tr class="import_essgrids">
			<td class="import_progress_item"><?php esc_html_e('Essential Grid', 'jacqueline'); ?></td>
