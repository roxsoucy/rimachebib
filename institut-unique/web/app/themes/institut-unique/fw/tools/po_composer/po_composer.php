...
// Theme init
if (!function_exists('jacqueline_po_composer_theme_setup')) {
	add_action( 'jacqueline_action_after_init_theme', 'jacqueline_po_composer_theme_setup' );		// Fire this action after load theme options
	function jacqueline_po_composer_theme_setup() {
		if (is_admin() && current_user_can('manage_options') && jacqueline_get_theme_option('admin_po_composer')=='yes') {
			new jacqueline_po_composer();
		}
	}
}


class jacqueline_po_composer {
	...
	function admin_menu_item() {
		if ( current_user_can( 'manage_options' ) ) {
			...
			jacqueline_admin_add_menu_item('theme', array(
				'page_title' => esc_html__('PO Composer', 'jacqueline'),
				'menu_title' => esc_html__('PO Composer', 'jacqueline'),
	...
	function load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			jacqueline_enqueue_style('po-composer-style', jacqueline_get_file_url('tools/po_composer/po_composer.css', array(), null));
		...
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			...
			jacqueline_enqueue_script('po-composer-script', jacqueline_get_file_url('tools/po_composer/po_composer.js', array('jquery'), null, true));
		}
	}
	...
	function build_page() {
		...
		$options  =  "\n".'<option value="upload_">'.esc_html__('Upload .po-file', 'jacqueline').'</option>'
					."\n".'<option value="edit_"'.(in_array($this->po_src, array('upload_', 'edit_')) ? ' selected="selected"' : '').'>'.esc_html__('Edit .po-text', 'jacqueline').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? esc_attr__('Parent languages', 'jacqueline') : esc_attr__('Languages', 'jacqueline')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->po_src))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.esc_attr__('Child languages', 'jacqueline').'">'.trim($this->get_list_options($child_list, 'child_', $this->po_src))."\n".'</optgroup>' : '');
		$options2 = "\n".'<option value="">'.esc_html__('- Select merge source -', 'jacqueline').'</option>'
					."\n".'<option value="upload_"'.($this->error && $this->po_src2=='upload_' ? ' selected="selected"' : '').'>'.esc_html__('Upload .po-file', 'jacqueline').'</option>'
					."\n".'<option value="edit_"'.($this->error && $this->po_src2=='edit_' ? ' selected="selected"' : '').'>'.esc_html__('Edit .po-text', 'jacqueline').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? esc_attr__('Parent languages', 'jacqueline') : esc_attr__('Languages', 'jacqueline')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.esc_attr__('Child languages', 'jacqueline').'">'.trim($this->get_list_options($child_list, 'child_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>' : '');

		?>
		<div class="po_composer">
			<h2 class="po_composer_title"><?php esc_html_e('Jacqueline .PO-files Composer', 'jacqueline'); ?></h2>
			...
			<form id="po_composer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php esc_html_e('Edit .po-file:', 'jacqueline'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src"><?php esc_html_e('Select .po-file:', 'jacqueline'); ?></label>
							...
							<div class="po_composer_field po_composer_editor">
								...
								<input type="checkbox" value="1" name="po_translated_down" id="po_translated_down"<?php echo isset($_POST['po_translated_down']) ? ' checked="checked"' : ''; ?> /><label for="po_translated_down"><?php esc_html_e('Move translated strings down', 'jacqueline'); ?></label>
								</span>
								<ul class="right_side">
									<li id="po_composer_link_text" class="po_composer_link_text"><a href="#po_composer_editor_text"><?php esc_html_e('Plain text', 'jacqueline'); ?></a></li><li id="po_composer_link_list" class="po_composer_link_list"><a href="#po_composer_editor_list"><?php esc_html_e('Strings editor', 'jacqueline'); ?></a></li>
								</ul>
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php esc_html_e('Merge with .po-file:', 'jacqueline'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src2"><?php esc_html_e('Select .po-file:', 'jacqueline'); ?></label>
				...
				<div class="po_composer_buttons">
					<a href="#" id="po_save"><?php echo empty($this->po_src) ? esc_html__('Upload', 'jacqueline') : esc_html__('Update', 'jacqueline'); ?></a>
					<?php if (!empty($this->po_link) || !empty($this->mo_link)) { ?>
					<div class="po_composer_links">
						<?php if (!empty($this->po_link)) { ?>
							<a href="<?php echo esc_url($this->po_link); ?>" class="po_composer_download_link"><?php esc_html_e('Download .PO-file', 'jacqueline'); ?></a>
						<?php } ?>
						<?php if (!empty($this->mo_link)) { ?>
							<a href="<?php echo esc_url($this->mo_link); ?>" class="po_composer_download_link"><?php esc_html_e('Download .MO-file', 'jacqueline'); ?></a>
						<?php } ?>
					</div>
					<?php } ?>
				</div>
		...
		if (!empty($this->po_src)) {
			if ( !wp_verify_nonce( jacqueline_get_value_gp('nonce'), admin_url() ) ) {
				...
				do {
					...
					if (($po_data = $this->parse_po($po_text))===false) {
						$this->error = esc_html__('Error parsing new .po-file!', 'jacqueline');
						break;
					}
					if ($this->po_src2 != '') {
						...
						if (($po_data2 = $this->parse_po($po_text2))===false) {
							$this->error = esc_html__('Error parsing merging .po-file!', 'jacqueline');
							break;
						}
					}
			...
			if (empty($this->error)) {
				...
				if (is_writeable($po_temp)) {
					jacqueline_fpc($po_temp, $this->po_text);
				...
				if (is_writeable($mo_temp)) {
					jacqueline_fpc($mo_temp, $this->mo_text);
				...
				if (empty($this->po_src2) && !empty($_POST['po_text']) && substr($this->po_src, 0, 7)=='parent_' || substr($this->po_src, 0, 6)=='child_') {
					...
					if (is_writeable($po_file)) {
						jacqueline_fpc($po_file, $this->po_text);
						$this->success = wp_kses_data( sprintf(__('Language file "<b>%s.po</b>" was updated!', 'jacqueline'), $name) );
						$mo_file = trim($dir).'/'.trim($name).'.mo';
						if (is_writeable($mo_file)) {
							jacqueline_fpc($mo_file, $this->mo_text);
							$this->success .= '<br>' . wp_kses_data( sprintf(__('Language file "<b>%s.mo</b>" was generated!', 'jacqueline'), $name) );
						}
					} else {
						$tjis->error_update = wp_kses_data( sprintf(__('Error writing in language file "<b>%s.po</b>"!<br>Please, copy text from textarea below and paste it in the .po-file manually<br>or use links to upload .po and .mo files below the text area!', 'jacqueline'), $name) );
					}
				}
			} else {
				$this->po_text = stripslashes($_POST['po_text']);
	...
	function load_po($suffix='') {
		...
		do {
			...
			if ($_POST['po_src'.trim($suffix)] == 'upload_') {
				$rez['data'] = isset($_FILES['po_file'.($suffix)]['tmp_name']) && file_exists($_FILES['po_file'.($suffix)]['tmp_name']) ? jacqueline_fga($_FILES['po_file'.($suffix)]['tmp_name']) : '';
				if (empty($rez['data'])) {
					$rez['error'] = sprintf(esc_html__('Error uploading or Empty .po-file: %s', 'jacqueline'), $_FILES['po_file'.($suffix)]['tmp_name']);
					break;
				}
			// or get content from textarea
				...
				if (!empty($_POST['po_text'.($suffix)]))
					...
					$rez['error'] = esc_html__('Empty textarea with .po-file content!', 'jacqueline');
					break;
				}
			// or load file from 'languages' folder
				...
				$name = jacqueline_esc(substr($_POST['po_src'.($suffix)], strpos($_POST['po_src'.($suffix)], '_')+1));
				$rez['data'] = file_exists(($dir).'/'.($name).'.po') ? jacqueline_fga(($dir).'/'.($name).'.po') : '';
				if (empty($rez['data'])) {
					$rez['error'] = sprintf(esc_html__('Error loading or Empty .po-file: %s', 'jacqueline'), ($dir).'/'.($name).'.po');
					break;
				}
