<?php
/**
 * Jacqueline Framework: social networks
 *
 * @package	jacqueline
...
// Theme init
if (!function_exists('jacqueline_socials_theme_setup')) {
	add_action( 'jacqueline_action_before_init_theme', 'jacqueline_socials_theme_setup' );
	function jacqueline_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'jacqueline_facebook_og_tags', 5 );
		}
...
jacqueline_storage_set('share_links', array(
...
//	'facebook' =>		'http://www.facebook.com/sharer.php?s=100&p[url]={link}&p[title]={title}&p[summary]={descr}&p[images][0]={image}',
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=jacqueline',
...
if (!function_exists('jacqueline_add_share_link')) {
	function jacqueline_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		jacqueline_storage_set('share_links', array_merge( jacqueline_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('jacqueline_show_share_links')) {
	function jacqueline_show_share_links($args) {
		if ( jacqueline_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'style' => jacqueline_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons',	// style for show icons: icons|images|bg
			'type' => 'block',					// share block type: list|block|drop
			'popup' => true,					// open share url in new window or in popup window
			'counters' => jacqueline_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => jacqueline_get_custom_option('show_share'),				// share block direction
			'caption' => jacqueline_get_custom_option('share_caption'),				// share block caption
			'share' => jacqueline_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				$icon = $args['style']=='icons' || jacqueline_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : jacqueline_get_socials_url(basename($soc['icon']));
				if ($args['style'] == 'icons') {
				...
				$url = $soc['url'];
				if (empty($url)) $url = jacqueline_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(jacqueline_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('jacqueline_prepare_socials')) {
	function jacqueline_prepare_socials($list, $style='') {
		if (empty($style)) $style = jacqueline_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || $style=='text' || jacqueline_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : jacqueline_get_socials_url(basename($soc['icon']));
				if ($style == 'icons' ||  $style=='text') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
					$sn = basename($soc['icon']);
...
if (!function_exists('jacqueline_get_twitter_data')) {
	function jacqueline_get_twitter_data($cfg) {
...
if (!function_exists('jacqueline_prepare_twitter_text')) {
	function jacqueline_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('jacqueline_get_twitter_followers')) {
	function jacqueline_get_twitter_followers($cfg) {
		$data = jacqueline_get_twitter_data($cfg); 
...
if (!function_exists('jacqueline_get_facebook_likes')) {
	function jacqueline_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(jacqueline_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (jacqueline_strpos($headers[0], '200')) {
			$json = jacqueline_fgc($url);
...
if (!function_exists('jacqueline_facebook_og_tags')) {
	function jacqueline_facebook_og_tags() {
		global $post;
		if ( !is_singular() || jacqueline_storage_get('blog_streampage')) return;
...
if (!function_exists('jacqueline_get_feedburner_counter')) {
	function jacqueline_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(jacqueline_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (jacqueline_strpos($headers[0], '200')) {
			$xml = jacqueline_fgc($url);
