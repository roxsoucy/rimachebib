...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		JACQUELINE_STORAGE['importer_error_messages'] = '';
		JACQUELINE_STORAGE['importer_ignore_errors'] = true;
		jacqueline_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function jacqueline_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: JACQUELINE_STORAGE['ajax_nonce'],
		action: 'jacqueline_importer_start_import',
	...
	jQuery.post(JACQUELINE_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || JACQUELINE_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				JACQUELINE_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				jacqueline_importer_do_action(steps, idx);
			} else {
				if (JACQUELINE_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + JACQUELINE_STORAGE['importer_error_msg'] + '</h4>' + JACQUELINE_STORAGE['importer_error_messages']);
				} else {
