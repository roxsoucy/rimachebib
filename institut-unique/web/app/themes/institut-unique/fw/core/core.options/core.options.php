	...
	function jacqueline_options_theme_setup() {
		...
		if ( is_admin() ) {
			...
			if ( jacqueline_options_is_used() ) {
				...
				//add_filter("jacqueline_filter_save_options",		'jacqueline_options_save_stylesheet', 10, 3);
	...
	function jacqueline_options_is_used() {
		...
		if (is_admin()) {
			if (isset($_REQUEST['action']) && ($_REQUEST['action']=='jacqueline_options_save' || $_REQUEST['action']=='jacqueline_options_import'))		// AJAX: Save or Import Theme Options
				$used = true;
			else if (jacqueline_strpos($_SERVER['REQUEST_URI'], 'jacqueline_options')!==false)															// Edit Theme Options
				$used = true;
			else if (jacqueline_strpos($_SERVER['REQUEST_URI'], 'post-new.php')!==false || jacqueline_strpos($_SERVER['REQUEST_URI'], 'post.php')!==false) {	// Create or Edit Post (page, product, ...)
				$post_type = jacqueline_admin_get_current_post_type();
				if (empty($post_type)) $post_type = 'post';
				$used = jacqueline_get_override_key($post_type, 'post_type')!='';
			} else if (jacqueline_strpos($_SERVER['REQUEST_URI'], 'edit-tags.php')!==false) {															// Edit Taxonomy
					...
					foreach ($inheritance as $k=>$v) {
						if (!empty($v['taxonomy']) && is_array($v['taxonomy'])) {
							foreach ($v['taxonomy'] as $tax) {
								if ( jacqueline_strpos($_SERVER['REQUEST_URI'], 'taxonomy='.($tax))!==false && in_array($post_type, $v['post_type']) ) {
									$used = true;
									break;
								}
							}
...
if ( !function_exists( 'jacqueline_load_custom_options' ) ) {
	function jacqueline_load_custom_options() {
		...
		if (!empty($inheritance_key)) {
			//$inheritance = jacqueline_get_theme_inheritance($inheritance_key);
...
if ( !function_exists( 'jacqueline_is_inherit_option' ) ) {
	function jacqueline_is_inherit_option($value) {
		...
		return jacqueline_strtolower($value)=='inherit';	//in_array(jacqueline_strtolower($value), array('default', 'inherit'));
	}
}
...
if ( !function_exists( 'jacqueline_options_load_scripts' ) ) {
	function jacqueline_options_load_scripts() {
		...
		jacqueline_enqueue_style( 'jacqueline-fontello-admin-style',	jacqueline_get_file_url('css/fontello-admin/css/fontello-admin.css'), array(), null);
		jacqueline_enqueue_style( 'jacqueline-fontello-style', 			jacqueline_get_file_url('css/fontello/css/fontello.css'), array(), null);
		jacqueline_enqueue_style( 'jacqueline-fontello-animation-style',jacqueline_get_file_url('css/fontello-admin/css/animation.css'), array(), null);
		// Jacqueline options styles
		jacqueline_enqueue_style('jacqueline-options-style',			jacqueline_get_file_url('core/core.options/css/core.options.css'), array(), null);
		jacqueline_enqueue_style('jacqueline-options-datepicker-style',	jacqueline_get_file_url('core/core.options/css/core.options-datepicker.css'), array(), null);

		// WP core media scripts
		wp_enqueue_media();

		// Color Picker
		//if (jacqueline_storage_get('to_colorpicker') == 'wp') {
			jacqueline_enqueue_style( 'wp-color-picker', false, array(), null);
			jacqueline_enqueue_script('wp-color-picker', false, array('jquery'), null, true);
		//} else if (jacqueline_storage_get('to_colorpicker') == 'tiny') {
			jacqueline_enqueue_script('jacqueline-colors-script',		jacqueline_get_file_url('js/colorpicker/colors.js'), array('jquery'), null, true );	
			//jacqueline_enqueue_style( 'jacqueline-colorpicker-style',	jacqueline_get_file_url('js/colorpicker/jqColorPicker.css'), array(), null);
			jacqueline_enqueue_script('jacqueline-colorpicker-script',	jacqueline_get_file_url('js/colorpicker/jqColorPicker.js'), array('jquery'), null, true );	
		//}

		// Input masks for text fields
		jacqueline_enqueue_script( 'jquery-input-mask',				jacqueline_get_file_url('core/core.options/js/jquery.maskedinput.1.3.1.min.js'), array('jquery'), null, true );	
		// Jacqueline core scripts
		jacqueline_enqueue_script( 'jacqueline-core-utils-script',		jacqueline_get_file_url('js/core.utils.js'), array(), null, true );	
		// Jacqueline options scripts
		jacqueline_enqueue_script( 'jacqueline-options-script',			jacqueline_get_file_url('core/core.options/js/core.options.js'), array('jquery', 'jquery-ui-core', 'jquery-ui-tabs', 'jquery-ui-accordion', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-datepicker'), null, true );	
		jacqueline_enqueue_script( 'jacqueline-options-custom-script',	jacqueline_get_file_url('core/core.options/js/core.options-custom.js'), array('jacqueline-options-script'), null, true );	
	...
	function jacqueline_options_prepare_scripts($override='') {
		...
		<script type="text/javascript">
			jQuery(document).ready(function () {
				JACQUELINE_STORAGE['to_delimiter']	= "<?php echo esc_attr(jacqueline_storage_get('to_delimiter')); ?>";
				JACQUELINE_STORAGE['to_slug']			= "<?php echo esc_attr(jacqueline_storage_get_array('to_flags', 'slug')); ?>";
				JACQUELINE_STORAGE['to_popup']		= "<?php echo esc_attr(jacqueline_get_theme_option('popup_engine')); ?>";
				JACQUELINE_STORAGE['to_override']		= "<?php echo esc_attr($override); ?>";
				JACQUELINE_STORAGE['to_export_list']	= [<?php
				...
				?>];
				if (JACQUELINE_STORAGE['to_strings']==undefined) JACQUELINE_STORAGE['to_strings'] = {};
				JACQUELINE_STORAGE['to_strings'].del_item_error			= "<?php esc_html_e("You can't delete last item! To disable it - just clear value in field.", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].del_item 				= "<?php esc_html_e("Delete item error!", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].recompile_styles			= "<?php esc_html_e("When saving color schemes and font settings, recompilation of .less files occurs. It may take from 5 to 15 secs dependning on your server's speed and size of .less files.", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].wait 					= "<?php esc_html_e("Please wait a few seconds!", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].reload_page				= "<?php esc_html_e("After 3 seconds this page will be reloaded.", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].save_options				= "<?php esc_html_e("Options saved!", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].reset_options			= "<?php esc_html_e("Options reset!", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].reset_options_confirm	= "<?php esc_html_e("Do you really want reset all options to default values?", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].reset_options_complete	= "<?php esc_html_e("Settings are reset to their default values.", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_options_header 	= "<?php esc_html_e("Export options", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_options_error		= "<?php esc_html_e("Name for options set is not selected! Export cancelled.", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_options_label		= "<?php esc_html_e("Name for the options set:", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_options_label2	= "<?php esc_html_e("or select one of exists set (for replace):", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_options_select	= "<?php esc_html_e("Select set for replace ...", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_empty				= "<?php esc_html_e("No exported sets for import!", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_options			= "<?php esc_html_e("Options exported!", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_link				= "<?php esc_html_e("If need, you can download the configuration file from the following link: %s", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].export_download			= "<?php esc_html_e("Download theme options settings", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].import_options_label		= "<?php esc_html_e("or put here previously exported data:", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].import_options_label2	= "<?php esc_html_e("or select file with saved settings:", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].import_options_header	= "<?php esc_html_e("Import options", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].import_options_error		= "<?php esc_html_e("You need select the name for options set or paste import data! Import cancelled.", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].import_options_failed	= "<?php esc_html_e("Error while import options! Import cancelled.", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].import_options_broken	= "<?php esc_html_e("Attention! Some options are not imported:", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].import_options			= "<?php esc_html_e("Options imported!", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].import_dummy_confirm		= "<?php esc_html_e("Attention! During the import process, all existing data will be replaced with new.", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].clear_cache				= "<?php esc_html_e("Cache cleared successfull!", 'jacqueline'); ?>";
				JACQUELINE_STORAGE['to_strings'].clear_cache_header		= "<?php esc_html_e("Clear cache", 'jacqueline'); ?>";
			});
		</script>
		<?php 
	}
...
if ( !function_exists( 'jacqueline_options_page_stop' ) ) {
	function jacqueline_options_page_stop() {
		echo trim(jacqueline_options_close_nested_groups('', true));
				?>
		...
		if (is_array($field['options']) && count($field['options']) > 0) {
			foreach ($field['options'] as $key => $item) {
				...
				if (in_array($field['type'], array('list', 'select', 'fonts')))
					...
					//$list .= '<img src="'.esc_attr($item).'" data-icon="'.esc_attr($key).'" alt="" class="jacqueline_options_input_image" />';
...
if ( !function_exists( 'jacqueline_options_show_field' ) ) {
	function jacqueline_options_show_field($id, $field, $value=null) {
		...
		if (jacqueline_options_is_group($field['type'])) {					// Close nested containers
			if (empty($field['start']) && (!in_array($field['type'], array('group', 'toggle')) || !empty($field['end']))) {
				echo trim(jacqueline_options_close_nested_groups($field['type'], !empty($field['end'])));
				if (!empty($field['end'])) {
					return;
				}
			}
			...
			if ($field['type'] != 'hidden') {
				...
				if ( !in_array($field['type'], array('divider'))) {
					echo '<label class="jacqueline_options_field_label'
						...
						. '>' 
						. ($field['title']) 
						. (!empty($field['info']) && ($fdir=jacqueline_get_file_url('images/to_info/'.jacqueline_esc($id).'.jpg'))!='' 
							? '<a href="'.esc_url($fdir).'" data-rel="popup" target="_blank" class="jacqueline_options_field_label_info iconadmin-info-circled" title="'.esc_html__('More info', 'jacqueline').'"></a>' 
							: '')
		...
		foreach ($field['val'] as $clone_num => $clone_val) {
			...
			default:
				if (function_exists('jacqueline_show_custom_field')) {
					echo trim(jacqueline_show_custom_field($id, $field, $clone_val));
				}
			} 
...
if ( !function_exists( 'jacqueline_options_save' ) ) {
	...
	function jacqueline_options_save() {
		...
		if ($mode == 'save') {
			...
			parse_str($_POST['data'], $post_data);
			if (!jacqueline_storage_empty('post_meta_box', 'fields')) {
				$options = jacqueline_array_merge(jacqueline_storage_get('options'), jacqueline_storage_get_array('post_meta_box', 'fields'));
			}
		} else
			$post_data = array();
