...
if (!function_exists('jacqueline_post_type_theme_setup')) {
	...
	function jacqueline_post_type_theme_setup() {
		...
		if ($override_key) {
			...
			add_action('add_meta_boxes',		'jacqueline_post_type_add_meta_box');
			add_action('admin_enqueue_scripts', 'jacqueline_post_type_admin_scripts');
			// Create meta box
			jacqueline_storage_set('post_meta_box', array(
				'id' => 'post-meta-box',
...
// Add meta box
if (!function_exists('jacqueline_post_type_add_meta_box')) {
	//add_action('add_meta_boxes', 'jacqueline_post_type_add_meta_box');
	function jacqueline_post_type_add_meta_box() {
		$mb = jacqueline_storage_get('post_meta_box');
		add_meta_box($mb['id'], $mb['title'], 'jacqueline_post_type_show_meta_box', $mb['page'], $mb['context'], $mb['priority']);
	}
}

// Callback function to show fields in meta box
if (!function_exists('jacqueline_post_type_show_meta_box')) {
	function jacqueline_post_type_show_meta_box() {
		global $post;
		...
		do_action('jacqueline_action_post_before_show_meta_box', $post_type, $post->ID);
		...
		do_action('jacqueline_action_post_after_show_meta_box', $post_type, $post->ID);
...
if (!function_exists('jacqueline_post_type_save_options')) {
	...
	function jacqueline_post_type_save_options($post_id) {
		...
		// verify nonce
		if ( !wp_verify_nonce( jacqueline_get_value_gp('meta_box_post_nonce'), admin_url() ) )
			return $post_id;
		...
		$post_options = array_merge(jacqueline_storage_get('options'), jacqueline_storage_get_array('post_meta_box', 'fields'));
