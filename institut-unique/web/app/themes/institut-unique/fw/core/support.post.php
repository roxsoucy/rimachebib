...
if (!function_exists('jacqueline_post_theme_setup')) {
	...
	function jacqueline_post_theme_setup() {
		...
		if (jacqueline_storage_get_array('post_meta_box', 'page') == 'post') {
			add_action('admin_enqueue_scripts', 						'jacqueline_post_admin_scripts');
			add_action('jacqueline_action_post_before_show_meta_box',		'jacqueline_post_before_show_meta_box', 10, 2);
			add_action('jacqueline_action_post_after_show_meta_box',		'jacqueline_post_after_show_meta_box', 10, 2);
...
if (!function_exists('jacqueline_post_after_theme_setup')) {
	add_action( 'jacqueline_action_after_init_theme', 'jacqueline_post_after_theme_setup' );
	function jacqueline_post_after_theme_setup() {
		// Update fields in the meta box
		if (jacqueline_storage_get_array('post_meta_box', 'page') == 'post') {
			jacqueline_storage_set_array('post_meta_box', 'fields', array(
					"partition_reviews" => array(
						"title" => esc_html__('Reviews', 'jacqueline'),
						"override" => "post",
						"divider" => false,
						"icon" => "iconadmin-newspaper",
						"type" => "partition"),
					"info_reviews_1" => array(
						"title" => esc_html__('Reviews criterias for this post', 'jacqueline'),
						"override" => "post",
						"divider" => false,
						"desc" => wp_kses_data( __('In this section you can put your reviews marks', 'jacqueline') ),
						"class" => "reviews_meta",
						"type" => "info"),
					"show_reviews" => array(
						"title" => esc_html__('Show reviews block',  'jacqueline'),
						"desc" => wp_kses_data( __("Show reviews block on single post page and average reviews rating after post's title in stream pages", 'jacqueline') ),
						"override" => "post",
						"class" => "reviews_meta",
						"std" => "inherit",
						"divider" => false,
					...
					"reviews_marks" => array(
						"title" => esc_html__('Reviews marks',  'jacqueline'),
						"override" => "post",
						"desc" => wp_kses_data( __("Marks for this review", 'jacqueline') ),
...
if (!function_exists('jacqueline_post_add_options_column')) {
	//add_filter('manage_edit-{post_type}_columns',	'jacqueline_post_add_options_column', 9);
...
if (!function_exists('jacqueline_post_fill_options_column')) {
	//add_filter('manage_{post_type}_custom_column',	'jacqueline_post_fill_options_column', 9, 2);
...
if (!function_exists('jacqueline_post_before_show_meta_box')) {
	//add_action('jacqueline_action_post_before_show_meta_box', 'jacqueline_post_before_show_meta_box', 10, 2);
	function jacqueline_post_before_show_meta_box($post_type, $post_id) {
		...
		<script type="text/javascript">
			jQuery(document).ready(function() {
				...
				JACQUELINE_STORAGE['reviews_levels']			= "<?php echo trim(jacqueline_get_theme_option('reviews_criterias_levels')); ?>";
				JACQUELINE_STORAGE['reviews_max_level'] 		= <?php echo (int) $max_level; ?>;
				JACQUELINE_STORAGE['reviews_allow_user_marks']= true;
			});
		</script>
...
if (!function_exists('jacqueline_post_after_show_meta_box')) {
	//add_action('jacqueline_action_post_after_show_meta_box', 'jacqueline_post_after_show_meta_box', 10, 2);
	function jacqueline_post_after_show_meta_box($post_type, $post_id) {
		?>
		</div>
		<?php
	}
...
if ( !function_exists( 'jacqueline_post_get_blog_title' ) ) {
	...
	function jacqueline_post_get_blog_title($title, $page) {
		...
		if ( $page == 'blog' )
			...
			$title = sprintf( esc_html__( '%s', 'jacqueline' ), single_cat_title( '', false ) );
		else if ( $page == 'tag' )
			...
			$title = jacqueline_get_post_title();		//$title = $wp_query->post->post_title;
		else
