...
if (!function_exists('jacqueline_wp_theme_setup')) {
	...
	function jacqueline_wp_theme_setup() {
		...
		add_action('wp_ajax_registration_user',				'jacqueline_callback_registration_user');
		add_action('wp_ajax_nopriv_registration_user',		'jacqueline_callback_registration_user');

		// AJAX: User login
		add_action('wp_ajax_login_user',					'jacqueline_callback_login_user');
		add_action('wp_ajax_nopriv_login_user',				'jacqueline_callback_login_user');

		// Allow user login by email from WP login dialog
		add_filter('authenticate',							'jacqueline_allow_email_login', 20, 3);
		...
		add_action( 'save_post', 							'jacqueline_clear_cache_all', 10, 2 );
		add_action( 'delete_post', 							'jacqueline_clear_cache_all', 10, 2 );
		add_action( 'edit_category', 						'jacqueline_clear_cache_categories',   10, 1 );
		add_action( 'create_category', 						'jacqueline_clear_cache_categories', 10, 1 );
		add_action( 'delete_category', 						'jacqueline_clear_cache_categories', 10, 1 );

		// Clear menu cache
		add_action('wp_update_nav_menu',					'jacqueline_clear_cache_menu', 10, 2);

		// Clear WP categories cache 
		add_action('wp_ajax_clear_cache',					'jacqueline_callback_clear_cache');
		add_action('wp_ajax_nopriv_clear_cache', 			'jacqueline_callback_clear_cache');
	}
}
...
if (!function_exists('jacqueline_show_pagination')) {
	function jacqueline_show_pagination($args=array()) {
		$args = array_merge(array(
			...
			'show_pages' => 5,
			'pages_in_group' => 10,
			'pages_text' => '', 		//esc_html__('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'jacqueline'),
...
if (!function_exists('jacqueline_get_calendar')) {
	function jacqueline_get_calendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		if ( $prev_year+$prev_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($prev_year) . '" data-month="' . esc_attr($prev_month) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'jacqueline'), $wp_locale->get_month($prev_month), date('Y', mktime(0, 0, 0, $prev_month, 1, $prev_year)))) . '">'
		...
		if ( $next_year+$next_month > 0 ) {
			$calendar_output .= '<a href="#" data-type="'.esc_attr(join(',', $posts_types)).'" data-year="' . esc_attr($next_year) . '" data-month="' . esc_attr($next_month) . '" title="' . esc_attr( sprintf(esc_html__('View posts for %1$s %2$s', 'jacqueline'), $wp_locale->get_month($next_month), date('Y', mktime(0, 0 , 0, $next_month, 1, $next_year))) ) . '">' 
...
if ( !function_exists( 'jacqueline_callback_calendar_change_month' ) ) {
	function jacqueline_callback_calendar_change_month() {
		...
		$response = array('error'=>'', 'data'=>jacqueline_get_calendar(false, $m, $y, array('post_type'=>$pt)));
...
if ( !function_exists( 'jacqueline_show_taxonomies_filters' ) ) {
	function jacqueline_show_taxonomies_filters() {
		...
		if ($page == 'post')
			...
			return;
		echo trim(jacqueline_get_terms_filters($taxes));
	}
}
...
if ( !function_exists( 'jacqueline_callback_clear_cache' ) ) {
	function jacqueline_callback_clear_cache() {
...
if (!function_exists('jacqueline_clear_cache')) {
	function jacqueline_clear_cache($cc) {
...
if ( !function_exists( 'jacqueline_clear_cache_all' ) ) {
	function jacqueline_clear_cache_all($post_id=0, $post_obj=null) {
...
if ( !function_exists( 'jacqueline_clear_cache_categories' ) ) {
	function jacqueline_clear_cache_categories($term_id=0) {
		delete_transient("jacqueline_terms_filter_category");
	}
}
...
if (!function_exists('jacqueline_get_post_data')) {
	function jacqueline_get_post_data(&$opt, $post_obj=null) {
		...
		//if (jacqueline_exists_wpml() && !empty($post) && is_object($post)) $cur_post = clone $post;
		$post_id = get_the_ID();
		...
		if ($opt['reviews']) {
			if (is_array($cached_post) && $cached_post['post_options_reviews']) {
				...
				$post_reviews_author = jacqueline_reviews_marks_to_display(get_post_meta($post_id, 'jacqueline_reviews_avg', true));
				$post_reviews_users  = jacqueline_reviews_marks_to_display(get_post_meta($post_id, 'jacqueline_reviews_avg2', true));
			}
		}
		...
		if ($post_format == 'gallery') {
			...
			if ($post_video=='') {
				...
				if (!jacqueline_is_youtube_url($src) && !jacqueline_is_vimeo_url($src))
					$src = '';
			} else {
			...
			if ($post_video!='' && $opt['substitute_video']) {	// && !jacqueline_in_shortcode_blogger(true)) {
				$post_video = jacqueline_substitute_video($post_video, $thumb_sizes['w'], round($thumb_sizes['w']/16*9), false);	//$thumb_sizes['h_crop']);
			}
			...
			if ($post_audio!='' && $opt['substitute_audio']) {	// && !jacqueline_in_shortcode_blogger(true)) {
...
if (!function_exists('jacqueline_get_post_views')) {
	function jacqueline_get_post_views($id=0){
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'jacqueline_post_views_count';
...
if (!function_exists('jacqueline_get_post_likes')) {
	function jacqueline_get_post_likes($id=0){
		global $wp_query;
		if (!$id) $id = $wp_query->current_post>=0 ? get_the_ID() : $wp_query->post->ID;
		$count_key = 'jacqueline_post_likes_count';
		$count = get_post_meta($id, $count_key, true);
		if ($count===''){
			delete_post_meta($id, $count_key);
			add_post_meta($id, $count_key, '0');
...
if (!function_exists('jacqueline_query_add_sort_order')) {
	function jacqueline_query_add_sort_order($args, $orderby='', $order='') {
		...
		if ($orderby == 'author_rating') {
			...
			$q['meta_key'] = 'jacqueline_reviews_avg';
			$q['meta_query'] = array(
				array(
					'meta_filter' => 'reviews',
					'key' => 'jacqueline_reviews_avg',
					'value' => 0,
					'compare' => '>=',
					'type' => 'NUMERIC'
				   )
			...
			$q['orderby'] = 'meta_value_num';
			$q['meta_key'] = 'jacqueline_reviews_avg2';
			$q['meta_query'] = array(
				array(
					'meta_filter' => 'reviews',
					'key' => 'jacqueline_reviews_avg2',
					'value' => 0,
					'compare' => '>=',
					'type' => 'NUMERIC'
				   )
			...
			$q['orderby'] = 'meta_value_num';
			$q['meta_key'] = 'jacqueline_post_views_count';
				...
				if (in_array($v, array('reviews', 'thumbs'))) {							// Filter with meta_query
					...
					if (!$found) {
						...
						if ($v == 'reviews') {
							$args['meta_query'][] = array(
								'meta_filter' => $v,
								'key' => 'jacqueline_reviews_avg'.(jacqueline_get_theme_option('reviews_first')=='author' ? '' : '2'),
								'value' => 0,
								'compare' => '>',
								'type' => 'NUMERIC'
							);
...
// AJAX: New user registration
if ( !function_exists( 'jacqueline_callback_registration_user' ) ) {
	// add_action('wp_ajax_registration_user',			'jacqueline_callback_registration_user');
	// add_action('wp_ajax_nopriv_registration_user',	'jacqueline_callback_registration_user');
	function jacqueline_callback_registration_user() {
	
		if ( !wp_verify_nonce( jacqueline_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			die();
	
		$user_name  = jacqueline_substr($_REQUEST['user_name'], 0, 20);
		$user_email = jacqueline_substr($_REQUEST['user_email'], 0, 60);
		$user_pwd   = jacqueline_substr($_REQUEST['user_pwd'], 0, 20);
		...
		if ( is_wp_error($id) ) {
		...
		} else if (($notify = jacqueline_get_theme_option('notify_about_new_registration'))!='no' && (($contact_email = jacqueline_get_theme_option('contact_email')) || ($contact_email = jacqueline_get_theme_option('admin_email')))) {
			$mail = jacqueline_get_theme_option('mail_function');
			if (in_array($notify, array('both', 'admin', 'yes'))) {
				$subj = sprintf(esc_html__('Site %s - New user registration: %s', 'jacqueline'), esc_html(get_bloginfo('site_name')), esc_html($user_name));
				$msg = "\n".esc_html__('New registration:', 'jacqueline')
			...
			if (in_array($notify, array('both', 'user', 'yes'))) {
				$subj = sprintf(esc_html__('Welcome to "%s"', 'jacqueline'), get_bloginfo('site_name'));
				$msg = "\n".esc_html__('Your registration data:', 'jacqueline')
					."\n".esc_html__('Name:', 'jacqueline').' '.esc_html($user_name)
					."\n".esc_html__('E-mail:', 'jacqueline').' '.esc_html($user_email)
					."\n".esc_html__('Password:', 'jacqueline').' '.esc_html($user_pwd)
...
// AJAX: Login user
if ( !function_exists( 'jacqueline_callback_login_user' ) ) {
	// add_action('wp_ajax_login_user',			'jacqueline_callback_login_user');
	// add_action('wp_ajax_nopriv_login_user',	'jacqueline_callback_login_user');
	function jacqueline_callback_login_user() {
	
		if ( !wp_verify_nonce( jacqueline_get_value_gp('nonce'), admin_url('admin-ajax.php') ) )
			die();
	
		$user_log = jacqueline_substr($_REQUEST['user_log'], 0, 60);
		$user_pwd = jacqueline_substr($_REQUEST['user_pwd'], 0, 20);
		$remember = jacqueline_substr($_REQUEST['remember'], 0, 7)=='forever';
...
if ( !function_exists( 'jacqueline_allow_email_login' ) ) {
	function jacqueline_allow_email_login( $user, $username, $password ) {
...
if ( !function_exists( 'jacqueline_get_nav_menu' ) ) {
	function jacqueline_get_nav_menu($slug='', $custom_walker=false) {
		...
		$list = jacqueline_get_theme_option('use_menu_cache')=='yes' ? get_transient('jacqueline_menu') : array();
		...
		if (empty($html)) {
			...
			if (!empty($slug) && jacqueline_get_theme_option('use_menu_cache')=='yes') {
				$list[trim($slug).'_'.trim($menu)] = $html;
				set_transient('jacqueline_menu', $list, 24*60*60);
			}
		}
		return $html;
	}
...
if ( !function_exists( 'jacqueline_clear_cache_menu' ) ) {
	function jacqueline_clear_cache_menu($menu_id=0, $menu_data=array()) {
		delete_transient("jacqueline_menu");
	}
}
